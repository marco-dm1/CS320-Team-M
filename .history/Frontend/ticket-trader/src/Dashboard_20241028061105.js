...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 h-full bg-white shadow-xl transition-all duration-300 z-50 
        ...
        <div className="p-6">
          <div className="flex items-center justify-between">
            ...
            >
              <ChevronDown className={`transform transition-transform ${!isSidebarOpen && 'rotate-90'}`} />
            </button>
          </div>
        </div>
        ...
        <nav className="mt-6">
          {[
            { icon: Ticket, label: 'My Tickets', active: true },
            ...
            <button
              key={index}
              className={`flex items-center w-full p-6 hover:bg-gray-50 transition-colors
                ${item.active ? 'text-primary border-r-4 border-primary bg-blue-50' : 'text-gray-600'}`}
            >
