...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {/* Sidebar */}
      <aside 
        className={`fixed lg:static inset-y-0 left-0 z-50 flex flex-col bg-white shadow-lg transition-transform duration-300 max-h-screen
          ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
          ${isSidebarOpen ? 'w-64' : 'w-20'}`}
      >
        <div className="p-6 flex-shrink-0">
          <div className="flex items-center justify-between">
            <Link to="/" className={`font-bold text-xl text-primary ${!isSidebarOpen && 'hidden'}`}>
              TicketTrading
            </Link>
            <button 
